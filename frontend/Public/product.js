...
let selectedPersonalizationOverlayImage = null;
let uploadedPersonalizationOverlayImage = null;
let uploadedPersonalizationOverlayObjectUrl = "";
...
function getPersonalizationAdjustmentSignature() {
    const text = personalizationAdjustments.text;
    const image = personalizationAdjustments.image;
...
function applyPersonalizationControlAction(target, action) {
    const adjustment = personalizationAdjustments[target];
...
function getSelectedPersonalizationOverlayImage() {
    return selectedPersonalizationOverlayImage;
}
...
async function preparePersonalizationImagePreview(file) {
    ...
    if (uploadedPersonalizationOverlayImage?.storageKey && window.personalizationImageStore?.deleteFile) {
        try {
            await window.personalizationImageStore.deleteFile(uploadedPersonalizationOverlayImage.storageKey);
        } catch (_error) {
            // Ignora falhas de limpeza local antes de substituir a imagem.
        }
    }

    if (uploadedPersonalizationOverlayObjectUrl) {
        URL.revokeObjectURL(uploadedPersonalizationOverlayObjectUrl);
        uploadedPersonalizationOverlayObjectUrl = "";
    }

    const storedImage = await window.personalizationImageStore.saveFile(file);
    const previewUrl = URL.createObjectURL(file);
    uploadedPersonalizationOverlayObjectUrl = previewUrl;
...
function updatePersonalizationPreview(product = currentProduct) {
    const { wrap, image, overlayImage, text } = getPersonalizationPreviewElements();
    ...
    const shouldShowPreviewText = productRequiresPersonalizationName(product);
    ...
    const applyPreviewTextStyles = () => {
        ...
        const scaledFontSize = previewConfig.fontSizePx * (currentWidth / referenceWidth);
    ...
    if (image.complete) {
        ...
        image.onload = applyPreviewTextStyles;
    }
...
function buildCartItemKey(product, personalizationName) {
    ...
    const overlayKey = selectedPersonalizationOverlayImage?.id || "";
    const adjustmentKey = getPersonalizationAdjustmentSignature();
...
async function calculateShippingQuote() {
    ...
    try {
        ...
        if (Array.isArray(result.options) && result.options.length) {
            ...
            showShippingFeedback("Nenhuma opção de PAC ou SEDEX foi encontrada para este CEP.", "error");
        }
    } catch (error) {
        renderShippingOptions([]);
        updateShippingHelpText("Consulte frete com PAC e SEDEX informando seu CEP.");
        updateShippingHelpText(buildShippingHelpText({
...
function renderProduct(product) {
    ...
    selectedPersonalizationOverlayImage = null;
    uploadedPersonalizationOverlayImage = null;
    if (uploadedPersonalizationOverlayObjectUrl) {
        URL.revokeObjectURL(uploadedPersonalizationOverlayObjectUrl);
        uploadedPersonalizationOverlayObjectUrl = "";
    }
...
function addCurrentProductToCart({ redirectToCheckout = false } = {}) {
    ...
    if (existingItem) {
        ...
        items.push({
            ...
            selectedVariations,
            personalizationName,
            personalizationImageUrl: selectedOverlayImage?.kind === "upload" ? "" : (selectedOverlayImage?.imageUrl || ""),
            personalizationImagePublicId: selectedOverlayImage?.kind === "upload" ? "" : (selectedOverlayImage?.imagePublicId || ""),
            personalizationImageStorageKey: selectedOverlayImage?.kind === "upload" ? (selectedOverlayImage?.storageKey || "") : "",
            personalizationImageKind: selectedOverlayImage?.kind || "",
            ...
            personalizationTextOffsetXPercent: personalizationAdjustments.text.offsetXPercent,
            personalizationTextOffsetYPercent: personalizationAdjustments.text.offsetYPercent,
            personalizationTextScalePercent: personalizationAdjustments.text.scalePercent,
            personalizationImageOffsetXPercent: personalizationAdjustments.image.offsetXPercent,
            personalizationImageOffsetYPercent: personalizationAdjustments.image.offsetYPercent,
            personalizationImageScalePercent: personalizationAdjustments.image.scalePercent,
    ...
    if (overlayOptions) {
        overlayOptions.addEventListener("click", (event) => {
            ...
            if (uploadedOptionButton) {
                ...
                selectedPersonalizationOverlayImage = { ...uploadedPersonalizationOverlayImage };
                renderPersonalizationImageOverlayOptions();
                updatePersonalizationPreview();
                updatePersonalizationControlVisibility();
                return;
            ...
            selectedPersonalizationOverlayImage = {
    ...
    if (overlayUploadInput) {
        overlayUploadInput.addEventListener("change", async () => {
            ...
            try {
                ...
                uploadedPersonalizationOverlayImage = {
